...
const horizontalDistance = ($position, $target) => {
    const p1 = $position.position();
    const p2 = $target.position();
    return Math.abs(p1.left - p2.left);
};
const verticalDistance = ($position, $target) => {
    const p1 = $position.position();
    const p2 = $target.position();
    return Math.abs(p1.top - p2.top);
};
const animationSpeed = (vertical, $position, $target) => {
    if (vertical) {
...
const appear = ($value) => {
    return new Promise((resolve) => {
        ...
        const helper = 'appear';
        prefixedEventListener_1.prefixedEventListener($value, 'animationend', () => {
...
jQuery(($) => {
    ...
    $value.css(topLeft());
    $('.js-control__appear').on('click', () => {
        handleClick(appear);
    });
    $('.js-control__destroy').on('click', () => {
        handleClick(destroy);
    });
    $('.js-control__top').on('click', () => {
        handleClick(moveTop);
    });
    $('.js-control__right').on('click', () => {
        handleClick(moveRight);
    });
    $('.js-control__bottom').on('click', () => {
        handleClick(moveBottom);
    });
    $('.js-control__left').on('click', () => {
